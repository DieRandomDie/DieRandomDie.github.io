...
function power(x) {
    ...
    let af = parseInt($('#armor-fest').val())/50
    let wc = parseInt($('#weapon-chant').val())/100
    let ac = parseInt($('#armor-chant').val())/100
    $('input.'+x).each(i=>{
    ...
    $(`#${x}-armor-power`).val(ap.toLocaleString())
    $('#percent-weapon-power').val((((equips.g.w/equips.c.w)-1)*100).toFixed(1) + '%')
    $('#percent-armor-power').val((((equips.g.a/equips.c.a)-1)*100).toFixed(1) + '%')
}
function update(e) {
...
window.onload = function () {
    ...
    if(localStorage['armor-chant']){$('#armor-chant').val(localStorage['armor-chant'])}
    if(localStorage['weapon-fest']){$('#weapon-fest').val(localStorage['weapon-fest'])}
    if(localStorage['armor-fest']){$('#armor-fest').val(localStorage['armor-fest'])}
